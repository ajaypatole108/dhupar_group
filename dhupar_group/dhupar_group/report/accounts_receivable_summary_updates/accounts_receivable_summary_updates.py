...
def execute(filters=None):
	...
	return AccountsReceivableSummaryUpdates(filters).run(args)

class AccountsReceivableSummaryUpdates(ReceivablePayableReport):
	def run(self, args):
	...
	def get_data(self, args):
		...
		for party, party_dict in iteritems(self.party_total):
			...
			query = "select customer_details from `tabCustomer` where name = \"" + str(party) + "\""
			row.customer_details = ""
			for d in frappe.db.sql(query):
				row.customer_details += str(d)
			row.industry = "<button type='button' class='btn btn-default ellipsis' data='"+ str(party) +"' onClick='updateRemark(this.getAttribute(\"data\"))''>Update</button>'"
	...
	def get_columns(self):
		...
		self.add_column(_('Remark'), fieldname='customer_details',width = '300px', fieldtype='Data')
		self.add_column(_('Button'), fieldname='industry', fieldtype='Data')
